...
[RESULTS.md](reports/RESULTS.md). This README documents the harnesses that produce them.

> [!TIP]
> Jump to [Running](#running) for copy-paste commands. The FAIR extraction-quality gate is `run_bench.py --quality`.
...
| Comparison | How does Argus stack up against Claude/Codex native web tools (breadth, depth, tokens, cost, speed)? | `run_compare.py`, `run_4way.py`, `scenarios.py`, `run_codex.sh`, `burst_test.py` | [RESULTS.md](reports/RESULTS.md) |
...
> [!NOTE]
> `report.md` is regenerated by `run_bench.py`; it is a build artifact, not a tracked record. The durable numbers live in [reports/RESULTS.md](reports/RESULTS.md).
...
| `semantic_ab.py` | Semantic rerank A/B: scores the same SearXNG candidate pool reranked lexical vs hybrid at nDCG@5 (see reports/RESULTS.md). |
...
> [!IMPORTANT]
> `quality_f1` is the gate to read; ROUGE-L is kept only for historical comparison.
...
<details>
<summary><strong>Honest consequence</strong> (why ROUGE-L now FAILS on the subset)</summary>

Against this neutral gold Argus no longer wins on ROUGE-L -
