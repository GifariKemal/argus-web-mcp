...
P0/P1/P2/P3 DONE. **DEPLOYED LIVE** on VPS `43.134.17.144` (Tencent, Ubuntu 24.04, 2 vCPU / 7 GB; replaced the dead `103.172.172.29` on 2026-09-15). Stack per host: uvicorn `127.0.0.1:8090 --workers 1`, SearXNG docker `:8888`, nginx, fail2ban. Public HTTPS `https://argus.gifariksuryo.xyz/mcp` (bearer auth) resumes once the DNS A record points at the new IP and certbot runs; until then the cert is a self-signed placeholder. Safe auto-update timer polls `main` every 5 min (ff-only -> health-gate -> auto-rollback; skips restart for docs/benchmark-only commits). Run locally: `docker compose up -d` (0.4.8 - Argus + SearXNG in Docker, MCP at `http://127.0.0.1:8090/mcp`, no auth, loopback only; the workstation MCP `argus` points here), or `./.venv/Scripts/python.exe -m argus.server` (stdio), or `uvicorn argus.server:app` (HTTP). Plans in `docs/plans/`.
...
## VPS access
SSH key-only: `ssh -i C:\Users\Administrator\.ssh\gifari_vps_ed25519 ubuntu@43.134.17.144` (Ubuntu 24.04; repo at `/opt/argus/app`). The old box `103.172.172.29` (user `ai`, Hermes + SUVA) is dead. Deployment is complete except TLS; owner inputs are the DNS A-record repoint, F1 `ARGUS_S2_API_KEY`, and optional SearXNG `outgoing.proxies`. Leave `ARGUS_ENABLE_LLM` unset unless intentionally enabling the optional LLM tier. Deploy runbook: `deploy/README.md`.
