...
## Status (updated 2026-08-10; deployed 2026-06-25)
P0/P1/P2/P3 DONE. **DEPLOYED LIVE** at `https://argus.gifariksuryo.xyz/mcp` (bearer auth) on VPS `103.172.172.29` (uvicorn `127.0.0.1:8090 --workers 1`, SearXNG docker `:8888`, LE TLS, nginx, fail2ban). Safe auto-update timer polls `main` every 5 min (ff-only -> health-gate -> auto-rollback; skips restart for docs/benchmark-only commits). Run locally: `docker compose up -d` (0.4.8 - Argus + SearXNG in Docker, MCP at `http://127.0.0.1:8090/mcp`, no auth, loopback only; the workstation MCP `argus` points here), or `./.venv/Scripts/python.exe -m argus.server` (stdio), or `uvicorn argus.server:app` (HTTP). Plans in `docs/plans/`.
