...
P0/P1/P2/P3 DONE. **DEPLOYED LIVE** at `https://argus.gifariksuryo.xyz/mcp` (bearer auth) on VPS `43.134.17.144` (Tencent, Ubuntu 24.04, 2 vCPU / 7 GB; replaced the dead `103.172.172.29` on 2026-09-15). **The VPS runs Argus as an Easypanel Compose service** off this repo's own `docker-compose.yml` (`argus` + `argus-searxng` containers); Easypanel's Traefik owns `:80`/`:443` and issues the Let's Encrypt cert, the panel sits on `:3000`. A GitHub push webhook redeploys `main`, so the old systemd unit, nginx vhost, certbot timer and `argus-update.timer` are all disabled on the host (kept on disk as a rollback path; `deploy/provision.sh` remains the non-Easypanel recipe). Run locally: `docker compose up -d` (0.4.8 - Argus + SearXNG in Docker, MCP at `http://127.0.0.1:8090/mcp`, no auth, loopback only; the workstation MCP `argus` points here), or `./.venv/Scripts/python.exe -m argus.server` (stdio), or `uvicorn argus.server:app` (HTTP). Plans in `docs/plans/`.
