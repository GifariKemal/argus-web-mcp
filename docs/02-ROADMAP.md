...
3. **Deploy to the SURIOTA VPS** - [x] **LIVE** on `43.134.17.144` since 2026-09-15, after `103.172.172.29` died. Runs as an Easypanel Compose service off the repo's `docker-compose.yml`: `argus` (uvicorn :8090, `--workers 1`) plus `searxng` (`http://searxng:8080`), no host ports published. Traefik terminates Let's Encrypt TLS; since 2026-09-18 the domain is Cloudflare-proxied and the public router only accepts Cloudflare ranges, with the rate limit keyed on `CF-Connecting-IP`. Secrets live in the Easypanel service env (`updateComposeEnv` with `createDotEnv: true`), never in the repo. The nginx + fail2ban + systemd path is retired but kept as `deploy/provision.sh`.
